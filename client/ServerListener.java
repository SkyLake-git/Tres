			...
			try {
				...
				if (size != -1) {
					...
					if (!this.client.isClosed()) {
						this.client.getLogger().warning("Socket closed by server (not DisconnectPacket)");
						this.client.close();
					}
				}
				...
				this.client.getLogger().emergency(Colors.wrap("Connection reset (SocketException)", Colors.RED_BACKGROUND));
				this.client.close();
			} catch (IOException e) {
